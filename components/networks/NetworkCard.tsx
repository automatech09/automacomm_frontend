...
import { IconAlertTriangle, IconCheck, IconPlugConnected, IconRefresh, IconUnlink } from "@tabler/icons-react";
import type { ConnectionStatus, NetworkCardProps } from "@/types";
...
export function NetworkCard({
  title, icon, iconBg, border, status, connected, main,
  onConnect, onDisconnect, onReconnect, reconnecting,
  showExpiredAlert, showViewButton,
  disconnectedText, connectLabel,
}: NetworkCardProps) {
  return (
    <Paper radius="xl" p="lg" style={{ border, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {connected ? (
        <Stack gap={12}>
          ...
          <Group gap="xs" mt="auto">
            {!showViewButton && (
              <Button size="xs" radius="md" bg="#04346D" leftSection={<IconRefresh size={13} />} loading={reconnecting} onClick={onReconnect} style={{ flex: 1 }}>
                Reconnecter
              </Button>
            )}
            <Button size="xs" radius="md" variant="light" color="red" leftSection={<IconUnlink size={13} />} onClick={onDisconnect} style={{ flex: 1, border: "1px solid rgba(239,68,68,0.18)", background: "rgba(239,68,68,0.06)" }}>
              Déconnecter
            </Button>
          </Group>
        </Stack>
        ...
        <Stack gap="sm" align="center" py="xs">
          ...
          <Button size="xs" radius="md" leftSection={<IconPlugConnected size={13} />} onClick={onConnect} loading={reconnecting} style={{ background: iconBg }}>
            {connectLabel}
          </Button>
        </Stack>
      )}
